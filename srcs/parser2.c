...
/*   Updated: 2022/03/18 14:55:25 by lbrianna         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_border(t_map *map, int i)
{
	...
	{
		if (map->map[0][i] != '1')
			ft_error("Border not rigth up");
	}
	i = -1;
	while (map->map[map->width - 1][++i])
	{
		if (map->map[map->width - 1][i] != '1')
			ft_error("Border not right down");
	}
	i = 0;
	while (map->map[++i] && i < map->width - 1)
	{
		...
		{
			if (map->map[i][0] != '1')
				ft_error("Border nor right left");
			if (map->map[i][map->length - 1] != '1')
				ft_error("Border not right right");
		}
	}
}
